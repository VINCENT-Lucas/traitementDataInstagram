...
def updateHoursCounter(msg, username, timeTableDic):
...
def treatment(filesList, username, wordCounterDic, sentMsgTimeTableDic, receivedMsgTimeTableDic):
  for file_path in filesList:
    with open(file_path, "r", encoding="utf-8") as file:
      json_data = json.load(file)
      conversationName = json_data["title"].encode('latin1').decode('utf-8')
      messages = json_data["messages"]

      for msg in messages:
        updateWordCounter(msg, username, wordCounterDic)
        updateHoursCounter(msg, username, sentMsgTimeTableDic) if msg["sender_name"] == username else updateHoursCounter(msg, username, receivedMsgTimeTableDic)
...
'''
def ConversationfilesProcess(jsonList, messageAmountDic, wordCounterDic, conversationAgeDic, dirName, userInfoDic, username, dirPaths):
    ...
    with open(os.path.join(dirPaths['Conversations'], dirName+'.html'), 'w', encoding="utf-8") as ConversationFile:
        ...
        for file_path in orderedDiscussions:
            with open(file_path, "r", encoding="utf-8") as file:
                ...
                updateMessageAmount(messageAmountDic, convData["conversationName"], messages)
                
                messagesTreatment(messages, ConversationFile, convData, calendarList, username, conversationAgeDic, conversationWordCounter)
    ...
    writeMostUsedWords(conversationWordCounter, wordCounterDic, os.path.join(dirPaths['Words'], dirName+'.html'), convData["conversationName"], dirName, dirPaths)

    writeDataConversation(dirName, convData, calendarList, dirPaths, username)

''' On effectue tous les traitements sur tous les messages.
C'EST ICI QU'ON ECRIT LES CONVERSATIONS.'''
def messagesTreatment(messages, ConvFile, convData, calendarList, username, conversationAge, conversationWordCounter):
    conversationName = convData["conversationName"]
    min = 1000*time.time()
    ancientDate = None
    for msg in messages[::-1]:
      messageDate = convert_timestamp(datetime.datetime.fromtimestamp(msg["timestamp_ms"]/1000))
      txt = ''
      ...
      if "content" in msg:
        ...
        if msg["sender_name"] == username:
          updateDict(conversationWordCounter, msg["content"].split(" "))
